...
pub async fn modify_req(code: &str, req: Request<Body>) -> Result<Request<Body>> {
    ...
    async_with!(context => |ctx| {
        ...
        let final_result = if result_val.is_promise() {
            // 等待 Promise 完成并获取返回值
            let promise = result_val.as_promise().unwrap().clone();
        ...
        if let Some(result_val) = final_result {
        ...
        log::warn!("JS 执行完成，但没有返回修改的对象，也没调用 $done");
